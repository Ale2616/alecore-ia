...
/**
 * ChatInput - Barra de entrada con glassmorphism y efectos
 */
const ChatInput = ({
  input,
  setInput,
  ...
  return (
    <div className="w-full">
      ...
      {attachedFiles.length > 0 && (
        <div className="flex flex-wrap gap-2 mb-3 pb-3 border-b border-surface-700/30">
          {attachedFiles.map((file, index) => (
            <div
              key={index}
              className="flex items-center gap-2 bg-surface-800/60
                         border border-surface-700/40 rounded-lg px-3 py-2
                         hover:border-surface-600/50 transition-colors group"
            >
              {file.type.startsWith('image/') && file.preview ? (
                ...
                <div className="w-8 h-8 flex items-center justify-center
                                bg-surface-700/50 rounded text-surface-400">
                  {getFileIcon(file)}
                </div>
              )}
      ...
      <div className="flex items-end gap-2 glass-panel rounded-2xl p-2
                      focus-within:border-accent-500/40 focus-within:shadow-glow-sm
        ...
        <button
          ...
          className="p-2.5 rounded-xl text-surface-400
                     hover:text-surface-200 hover:bg-surface-700/40
        ...
        <textarea
          ...
          rows={1}
          className="flex-1 bg-transparent text-surface-50 placeholder-surface-500
                     resize-none py-2.5 px-2 focus:outline-none scrollbar-thin
                     max-h-[140px] min-h-[40px] text-sm leading-relaxed"
          disabled={isLoading}
        />
